...
Slide.prototype.init = function(){
	// 사용자가 지정한 stage에 css적용
	this.$container.css({
		"position":"relative",
		"overflow":"hidden"
	});

	// stage에 슬라이드를 감싸는 woo-wrapper를 생성
	this.$wrapper = $('<div class="woo-wrapper woo-'+this.direction+'"></div>').appendTo(this.$container);
	
	// woo-wrapper에 woo-wrapper들을 만드는 과정
	for(var i=0, html=''; i<this.slide.length; i++){
		if(this.direction == 'step')
		html  = '<div class="woo-slide" style="background-image: url(\''+this.slide[i]+'\');">';
		else
		html  = '<div class="woo-slide">';
	html += '<img src="'+this.slide[i]+'" class="w100">';
	if(this.title && this.title[i])html += this.title[i];
	html += '</div>';


	// hory, vert 형은 woo-wrapper에 실제 테그를 넣어놔야 되므로
	// 1. 실제태그도 붙이고
	// 2. this.$slide배열에도 추가한다.
	if(this.direction === 'hori' || this.direction == 'vert') {this.$slides.push($(html).appendTo(this.$wrapper));
	}

	// fade 형은 step형은 this.$slides에만 태그를 넣어놓는다.
	if(this.direction === 'fade' || this.direction == 'step')this.$slides.push($(html));
}
// for 끝.


// hory, vert type은 0번태그를 복제하여 마지막에 붙여 놓는다.
...
// fade type은 0번 슬라이드를 woo-wrapper에 붙인다.
...
if (this.btnUse){
  this.$btnPrev = $('<div class="woo-btn woo-prev">〈</div>').appendTo(this.$container);
	this.$btnNext = $('<div class="woo-btn woo-next">〉</div>').appendTo(this.$container);
...
if (this.pagerUse) {
	...
	// woo-pagers를 this.$container에 생성한다.
	this.$pagers = $('<div class="woo-pagers"></div>').appendTo(this.$container);

  // 각각의 pager를 생성하고 클릭 이벤트를 적용한다.
	for(var i in this.slide){
		$('<div class="woo-pager"></div>').appendTo(this.$pagers).click(this.onPagerClick.bind(this));
	}

	// 0번 idx에 .active 클래스를 적용한다.
	this.$pagers.find(".woo-pager").eq(0).addClass("active");
}
...
if(this.autoUse){
 ...
 this.$container.append(html);
	...
	$(window).resize(function(){
		...
		this.$wrapper.find('woo-slide').css({
...
// this.$container의 mouseover이벤트 콜백
Slide.prototype.onMouseOver = function(){
	clearInterval(this.interval);
}
// this.$container의 mouseleave이벤트 콜백
Slide.prototype.onMouseLeave = function(){
	this.interval = setInterval(this.onInterval.bind(this), this.gapSpeed);
}
// woo-wrapper의 animation
Slide.prototype.ani = function(){
	if(this.pagerUse){
	  this.$pagers.find(".woo-pager").eq(this.now).addClass("active").siblings().removeClass("active");
	  if(this.direction === 'hori' || this.direction === 'vert'){
		if(this.now == this.last) this.$pagers.find(".woo-pager").eq(0).addClass("active").siblings().removeClas("active");
	  }
	}
...
if(this.direction === 'fade'){
  ...
  $(this).prev().remove();
		...
		Slide.prototype.stepCalc = function(cnt) {
			...
			this.slideValue.parentWidth = (Math.abs(stepValue.leftValue)*2+100).toFixed(4);
